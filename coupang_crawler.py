...
class CoupangCrawler:
    ...
    def _extract_from_html(self, html: str) -> Tuple[int, List[float], List[float]]:
        ...
        reviews: List[float] = []
        for li in products:
            if li.select_one(".search-product__ad"):
                continue
            price_el = li.select_one(".price-value")
            review_el = li.select_one(".rating-total-count")
            if not price_el or not review_el:
                continue
            p = self._parse_int(price_el.get_text(strip=True))
            r = self._parse_int(review_el.get_text(strip=True))
...
if __name__ == "__main__":
    print(crawl_coupang("페이스 리프팅 밴드"))
